...
これだけ大きな会社なんだから、ドキュメントを書くときの全社的な汎用ルールくらいあるのでは？と思っても、意外とないことが多い。それってなんでなんですか？と聞かれたので考えてみた。
...
たとえばネットショップに割引券を作るAPIがあったとして、そのドキュメントを書くとき、「discount_rate」というフィールドを単純に「割引率」として、「割引率を1～9の数字で指定します。たとえば3を指定すると3割引になります」と説明したとする。このAPIを使ってネットショップを運営する対象のリージョンが日本だけならこれで問題ないと思う。

でもそのAPI群が実はグローバルなサービスで、すべての機能はローカライズされており、もし3を指定したら台湾のショップでは「打3折」と表示されて3割の価格で買える、つまり7割引になるんだとしたら、「3を指定すると3割引になります」という説明は嘘になってしまう。1割引にしたつもりが9割引になっていたら、ネットショップ運営者はカンカンに怒るはずだ。
